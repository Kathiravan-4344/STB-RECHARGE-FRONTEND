...
export function ChatWidget({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function send() {
    ...
    setText("");
    setTimeout(() => {
      setMsgs((x) => [...x, { from: "op", text: "Got it! I'm checking your STB status now…", t: Date.now() }]);
    }, 900);
  }
    ...
    <div className="fixed bottom-40 right-4 z-50 w-[min(360px,calc(100vw-2rem))] md:bottom-24">
      <div className="glass-strong overflow-hidden rounded-2xl border border-white/10">
        ...
        <div className="max-h-80 space-y-2 overflow-y-auto p-3">
          {msgs.map((m, i) => (
            <div key={i} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${m.from === "me" ? "gradient-primary text-primary-foreground" : "bg-white/5 text-foreground border border-white/10"}`}>
                {m.text}
              </div>
            </div>
          ))}
        ...
        <div className="flex items-center gap-2 border-t border-white/10 p-2">
          ...
          />
          <button onClick={send} className="grid h-9 w-9 place-items-center rounded-xl gradient-primary text-primary-foreground">
